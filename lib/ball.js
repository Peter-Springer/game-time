...
class Ball {
  ...
  collisionWithBrickRight(brickX, brickY, brickHeight, brickWidth) {
      if (this.x < brickX + brickWidth && this.x > brickX + (brickWidth - 4) && this.y < brickY + brickHeight && this.y + this.height > brickY ) {
        return this.speedx = -this.speedx
      }
  };

  collisionWithBrickLeft(brickX, brickY, brickHeight, brickWidth) {
    if (this.x + this.width > brickX && this.x + this.width < brickX + 4 && this.y < brickY + brickHeight && this.y + this.height > brickY)
    return this.speedx = -this.speedx
  }

  collisionWithBrickTop(brickX, brickY, brickHeight, brickWidth) {
    if (this.y + this.height > brickY &&  this.y + this.height < brickY + 4 && this.x + this.width > brickX && this.x < brickX + brickWidth)
    return this.speedy = -this.speedy
  }

  collisionWithBrickBottom(brickX, brickY, brickHeight, brickWidth) {
    if (this.y < brickY + brickHeight &&  this.y > brickY + (brickHeight - 4) && this.x + this.width > brickX && this.x < brickX + brickWidth)
    return this.speedy = -this.speedy
  }
};
